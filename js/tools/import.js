GW.define('Tool.IMPORT', 'tool', {
	...
	run: function() {
		...
		if (args.length == 0) {
			return Defer.rejected('args', "You must specify <links> to import.");
		}
		...
		if (fileLinks.concat(folderLinks).length == 0) {
			return Defer.rejected('nop', "No valid links found.");
		}
		...
		return this.getSession().then(function(session) {
			...
			return Defer.when(calls).then(function() {
